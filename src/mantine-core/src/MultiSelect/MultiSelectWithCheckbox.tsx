...
import { filterData } from './filter-data/filter-data-for-checkbox';
...
import useMultiSelectCheckboxStyles from './MultiSelectWithCheckbox.style';
...
export const MultiSelectWithCheckbox = forwardRef<HTMLInputElement, MultiSelectProps>(
  (props, ref) => {
    const {
      className,
      style,
    ...
    const { classes: cbClasses } = useMultiSelectCheckboxStyles();
    ...
    const handleInputChange = (event: React.ChangeEvent<HTMLInputElement>) => {
      ...
      console.log('handleInputChange: ', !disabled && !valuesOverflow.current && searchable);
      !disabled && !valuesOverflow.current && searchable && setDropdownOpened(true);
    };

    const handleInputFocus = (event: React.FocusEvent<HTMLInputElement>) => {
      typeof onFocus === 'function' && onFocus(event);
      console.log('handleInputFocus: ', !disabled && !valuesOverflow.current && searchable);
      !disabled && !valuesOverflow.current && searchable && setDropdownOpened(true);
    };
    ...
    const firstSelectedItem =
      selectedItems != null && selectedItems.length ? selectedItems[0] : undefined || {};
    const ValueRenderer = Value as React.FC;
          ...
          <SelectPopover.Target>
            ...
            >
              ...
              <Input<'div'>
                ...
                onMouseDown={(event) => {
                  event.preventDefault();
                  console.log('onMouseDown: ', !disabled && !valuesOverflow.current);
                  ...
                  {firstSelectedItem.value && (
                    ...
                    >
                      {selectedItems.length > 1 && (
                        <span className={cbClasses.restSelectedItems}>
                          &nbsp;+{selectedItems.length - 1}
                        </span>
                      )}
                    </ValueRenderer>
...
MultiSelectWithCheckbox.displayName = '@mantine/core/MultiSelectWithCheckbox';
